...
def adapter_train(train_loader,optimizer,adapter,criterion,args, wandb=None):
    ...
    for epoch in range(args['epochs']):
        ...
        for batch_num, input_data in enumerate(train_loader):
            ...
            # if wandb:
            #     wandb.log({'adapter_loss': loss.item(), 'adapter_lr': learning_rate, 'adapter_epoch': epoch, 'adapter_batch': batch_num})
...
def linear_eval_all(model, train_loader, test_loader, args, inds, amts, grid_search=False, target_type='target'):
    ...
    # x_train_o = x_train_o.astype(np.float16)
    # x_test_o = x_test_o.astype(np.float16)
    ...
    with torch.no_grad():
        for data in loader:
            if args['half_precision']:
                data['im'] = data['im'].to(args['device']).half()
            else:
