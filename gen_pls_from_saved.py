...
def main(args):
    ...
    pred_df = pd.read_csv(f'data/imagenet/imagenet_zs_prob_preds.csv')
    # pred_df = pd.read_csv('/l/users/sanoojan.baliah/Felix/svl_adapter/imagenet_zs_preds.csv')
    # pred_df = pred_df.rename(columns={'target': 'label'})
    predicted_labels = set(pred_df.pred1)
    label_to_category = dict(meta[['label', 'category_id']].drop_duplicates().values)
    ...
    list_of_classes_without_pseudolabel = set(pred_df.label) - predicted_labels
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset',required=True, choices=['resisc45', 'aid', 'patternnet', 'whurs19', 'ucm', 'optimal31', 'mlrsnet',
    ...
    parser.add_argument("--imgs_per_label", type=int,
                        help='the amount of pseudolabels to keep for each of the predicted labels '
                             '(ranked based on their clip confidence, higher first)', default=16, required=True)
