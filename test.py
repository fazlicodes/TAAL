...
def main(args):
    ...
    if args['dataset']=='imagenet':
        ...
        prefix = 'a centered satellite image of '
    else:
    ...
    grouped = pred_df2.groupby('pred1').mean().reset_index().rename(columns={'pred1': 'pred1', 'prob1': 'avg_prob1'})
    grouped = grouped[['pred1', 'avg_prob1']]
    pred_df = pred_df.merge(grouped, on='pred1')
    pred_df['quartile_75'] = pred_df['avg_prob1']*0.75
    ...
    for pred_label in predicted_labels:
        ...
        max_rows=min_rows*2
        rows_to_select = max(min_rows, min(int(0.1 * len(sub_label_df)),max_rows))

        sub_label_df = sub_label_df.head(rows_to_select)
        pseudo_df = pd.concat((pseudo_df, sub_label_df))

    #Remove columns 'avg_prob1','quartile_75'
    pseudo_df.drop(['avg_prob1','quartile_75'], axis=1, inplace=True)
