...
def main(args):
    ...
    if args['pseudolabel_model']=="georsclip":
        model_name = 'georsclip'

    if args['pseudolabel_model']=="clip_ViT-B_32":
        model_name = 'ViT_B32'
    
    if args['use_pseudo']==True:
        args['metadata'] = os.path.join(args['data_dir'], args['dataset']+'_meta_{}_pseudo_clip_{}.csv'.format(model_name,args['pseudo_conf']))
    else:
    ...
    if args['model_type'] == "clip":
        ...
        if args['model_subtype']=="pretrained_model/GeoRSCLIP/RS5M_ViT-B-32.pt":
            args['model_subtype'] = 'georsclip'
        args['model_subtype'] ='{}_{}'.format(args['model_type'],args['model_subtype'])
        ...
        if args['model_path']!='na':
            ...
            list_of_models = list(Path(args['model_dir']).glob('*.pt')) # list all models in the dir (*.pt)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_subtype",type=str,default="", help="exact type of pretraining backbone")
    ...
    parser.add_argument("--pseudolabel_model",type=str,choices=['georsclip','clip_RN50','clip_ViT-L_14','clip_ViT-B_32'])
