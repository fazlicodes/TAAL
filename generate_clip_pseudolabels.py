...
def main(args):
    ...
    elif args['dataset'] in ['aid','eurosat','patternnet','resisc45','ucm','whurs19','mlrsnet','optimal31']:
        # text_pr = "This is an aerial image of a "
        # text_pr = "a land use image of "
        # text_pr = "a satellite image of "
    ...
    #map datasetnet name based on the dataset name
    dataset = map_datasets_name[datasets_names.index(args['dataset'])]

    ### Load CLIP model
    # model, preprocess = clip.load(args['model_subtype'], device=device, jit=True)
    model, preprocess = clip.load(args['model_subtype'], device=device, jit=False)

    # datasets_names = ['eurosat','ucm','aid','patternnet','resisc45','whurs19','mlrsnet','optimal31', 'caltech-101']
    ...
    for i in range(N):
        ...
        with torch.no_grad():
            ...
            logits_per_image,logits_per_text2 = model.forward_with_text_desc(image, classifier_weights)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', choices=['ucf101','food101','sun397','stanford_cars','fgvc-aircraft','caltech-101','eurosat',
                                            'resisc45','aid','patternnet', 'whurs19','ucm','optimal31', 'mlrsnet',
